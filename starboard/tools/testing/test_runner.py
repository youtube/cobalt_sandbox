...
class TestRunner(object):
  ...
  def GenerateCoverageReport(self):
    ...
    toolchain_dir = build._GetClangBinPath(clang.GetClangSpecification())

    report_name = "report"
    profdata_name = os.path.join(self.coverage_directory,
                                 report_name + ".profdata")
    merge_cmd_list = [
        os.path.join(toolchain_dir, "llvm-profdata"), "merge", "-sparse=true", "-o", profdata_name
    ]
    merge_cmd_list += available_profraw_files

    print(merge_cmd_list)
    self._Exec(merge_cmd_list)
    show_cmd_list = [
        os.path.join(toolchain_dir, "llvm-cov"), "show", "-instr-profile=" + profdata_name, "-format=html",
    ...
    report_cmd_list = [
        os.path.join(toolchain_dir, "llvm-cov"), "report", "-instr-profile=" + profdata_name,
        available_targets[0]
    ]
